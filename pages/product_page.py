...
class ProductPage(BasePage):
    ...
    def is_book_title_presented_in_message(self, book_title):
        assert book_title == self.browser.find_element(*ProductPageLocators.BOOK_ADDED_TO_BASKET).text, "Incorrect book title"

    def is_basket_cost_equals_book_cost(self, book_cost):
        assert book_cost in self.browser.find_element(*ProductPageLocators.BOOK_PRICE).text, "Incorrect book cost"

    def book_added_message_is_not_presented(self):
        assert self.is_not_element_present(*ProductPageLocators.BOOK_ADDED_TO_BASKET), "Success message presented"

    def book_added_messsage_disappeared(self):
        assert self.is_disappeared(*ProductPageLocators.BOOK_ADDED_TO_BASKET), "Message does not disappeared"
